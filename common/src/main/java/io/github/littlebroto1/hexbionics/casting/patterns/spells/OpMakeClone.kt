package io.github.littlebroto1.hexbionics.casting.patterns.spells
...
class OpMakeClone : SpellAction {
    ...
    override fun execute(args: List<Iota>, ctx: CastingContext): Triple<RenderedSpell, Int, List<ParticleSpray>> {
        ...
        if (target !is PlayerEntity) throw MishapBadEntity(
            target,
            Text.translatable("text.hexbionics.congrats.player")
        )
     ...
     */
    private data class Spell(val target: PlayerEntity) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            /*val entity = object : PlayerEntity(ctx.world, ctx.caster.blockPos, ctx.caster.yaw, GameProfile(UUID.randomUUID(), ctx.caster.gameProfile.name), null) {
            ...
            val entity = PlayerHusk(ctx.caster);

            ctx.caster.teleport(ctx.world, target.x, target.y, target.z, target.yaw, target.pitch)

            target.remove(Entity.RemovalReason.DISCARDED);

            //entity.copyFrom(ctx.caster);

            ctx.world.spawnEntity(entity);

            //ctx.caster.sendMessage(Text.translatable("text.hexbionics.congrats", player.displayName));
        }
    }
}